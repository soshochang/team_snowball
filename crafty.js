...
/**@
	...
	*/
    ...
    /**@
    * #.animate_y
    * @comp SpriteAnimation for Y-axis sprite sheets
    */
    animate_y: function (reelId, fromy, x, toy) {
        var reel, i, tile, tileh, duration, pos;

        //play a reel
        //.animate('PlayerRunning', 15, -1) // start animation
        if (arguments.length < 4 && typeof fromy === "number") {
            duration = fromy;
            ...
            if (arguments.length === 3 && typeof x === "number") {
                //User provided repetition count
                if (x === -1) this._frame.repeatInfinitly = true;
                else this._frame.repeat = x;
            }
        ...
        if (typeof fromy === "number") {
            ...
            reel = [];
            toy = fromy.length - 1;
            tile = this.__tile + parseInt(this.__padding[1] || 0, 10);
            tileh = this.__tileh + parseInt(this.__padding[0] || 0, 10);

            for (; i <= toy; i++) {
                pos = fromy[i];
                reel.push([pos[1] * tile, pos[0] * tileh]);
            }
