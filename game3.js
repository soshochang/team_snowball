window.onload = function() {
	...
	Crafty.scene("main", function() {
		...
		//here I attached an event Listener. You can listen for an event from Node and cause the player to move
		$(document).keydown(function(e) {
		  // console.log(e);
		  	if(e.which == 37 || e.which == 38 || e.which == 39 || e.which == 40)
		  	{	
			  	player_pos_array = [player1.x, player1.y];
			  	// console.log("keypress down");
			  	// console.log(player1.speed);
			  	// console.log(e);
			  	count_sent++;
			  	console.log('Count: ' + count_sent);
				socket.emit('send_player_pos', player_pos_array);
				// socket.emit('client_keydown', e.which);
			}	
		...
		// Eug added this to increase rate of position updates
		$(document).keyup(function(e) {
			// player_pos_array = [player1.x, player1.y]
			// console.log(player1.speed);
			// socket.emit('send_player_pos', player_pos_array);
			// socket.emit('client_keyup', e.which);
		});
		...
		new_user_properties = new Object();

		
		//create our player entity with some premade components
		new_user_properties.createplayer = function(socket_username) {player1 = Crafty.e("2D, Canvas, player, Controls, LeftControls, SpriteAnimation, Collision, BombDropper, Grid")
			...
			.bind("NewDirection", function(direction) {
				if(direction.x < 0 ) {
					...
					//this is where you can detect events from the client and send to Node Server
		...
		// 	return this;
			...
			player1.username = socket_username; 
